...
export default function Resume() {
  ...
  const jobsList = jobs.map((x) => {
    const bulletpoints = x.description.map((x) => <li key={`jobs-${uuid()}`}>{x}</li>);
  ...
  const projectsList = projects.map((x) => {
    const bulletpoints = x.description.map((x) => <li key={`projects-${uuid()}`}>{x}</li>);
  ...
  const skillsList = skills.map((x) => (
    <li key={`skills-${uuid()}`} className="prose-sm m-1 rounded-md bg-primary px-2.5 py-1 font-semibold text-white">
      {x}
    </li>
  ));
    ...
    <section id="resume" className="h-full w-full scroll-mt-16 overflow-hidden bg-base-100">
      <Container className="h-full">
        ...
        <ul className="flex max-w-5xl list-disc flex-col justify-center space-y-20 px-12 py-20">
          ...
          {divider}
          <li key={`education-section-${uuid()}`} className="resume-content-item flex justify-start gap-2">
